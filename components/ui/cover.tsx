...
export const CircleIcon = ({ className }: { className?: string; delay?: number }) => {
  return (
    <div
      className={cn(
        `group pointer-events-none h-2 w-2 animate-pulse rounded-full bg-primary opacity-20 group-hover/cover:hidden group-hover/cover:bg-white group-hover/cover:opacity-100`,
        className
      )}
    ></div>
  );
