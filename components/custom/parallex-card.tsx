...
export default function ParallexCard({ type, src }: ParallexCardProps) {
  const backgroundClass = {
    ...
    grid: 'dark:bg-grid-white/[0.3] bg-grid-black/[0.2] border border-zinc-100 dark:border-zinc-900 rounded-xl',
      ...
      <CardBody className={`group/card relative h-auto w-auto p-6 sm:w-[30rem] ${backgroundClass}`}>
        <CardItem translateZ="100" className="mt-4 w-full">
          <Image
            ...
            height="1000"
            width="1000"
            className="h-full w-full rounded-lg border border-zinc-100 object-cover group-hover/card:shadow-xl dark:border-zinc-900"
            alt="thumbnail"
          />
        </CardItem>
      </CardBody>
